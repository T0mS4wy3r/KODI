...
kodi_release = xbmc.getInfoLabel("System.BuildVersion") # returns 18.9 in osmc
kodi_version = re.findall('&&(.*?)[ -]','&&'+kodi_release,re.DOTALL) # array is empty and causes error out of range
kodi_version = float(kodi_version[0] if kodi_version else kodi_release) # has fixed the problem for me, maybe a better way to modify the regex. (shokran akhi emad)
