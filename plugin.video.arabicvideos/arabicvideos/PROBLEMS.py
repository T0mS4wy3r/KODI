...
def MAIN(mode):
	if mode==150:
		...
		message = 'مؤخرا قامت بعض شركات الانترنيت الدولي بوضع عائق امام البرامج مثل كودي لتسمح فقط لبعض مستخدمي المتصفح بالدخول لمواقع الفيديو وهذا العائق هو reCAPTCHA الخاص بشركة جوجل والمصمم خصيصا لمنع البرامج من تصفح الانترنيت'
		message += '\n\n'+ 'ونتيجة لهذا العائق فانه تقريبا جميع مستخدمي برنامج كودي لا يستطيعون الدحول لجميع مواقع هذا البرنامج حتى مع استخدام VPN او Proxy او DNS او اي حل بسيط اخر لان هذا لن يحل المشكلة وانما فقط سيقوم باصلاح بعض المواقع واعاقة مواقع اخرى كانت تعمل سابقا بدون مشاكل'
		xbmcgui.Dialog().textviewer('مشكلة بعض المواقع لا تعمل',message)
		message = 'المواقع التي تأثرت بالعائق عند بعض الناس هي:'
		message += '\n'+'shahid4u  egy4best  movizland  akoam  series4watch  egybest'
		message += '\n\n'+'الدول التي تأثرت بالعائق عند بعض الناس هي:'
		message += '\n'+'مصر  الكويت  اميركا  كندا  فرنسا  اليونان  بريطانيا'
		message += '\n\n'+'لقد وجد المبرمج طريقة لتجاوز العائق ولكنها تحتاج جهد كبير والمبرمج يظن المشكلة صغيرة ولا تستحق التعب فاذا كانت لديك مشكلة بالدخول لبعض المواقع فاذن ارسل رسالة للمبرمج تكتب فيها اسم بلدك واسماء المواقع التي لا تستطيع دخولها لكي يتضح حجم المشكلة'
		xbmcgui.Dialog().textviewer('مشكلة بعض المواقع لا تعمل',message)
		import PROGRAM
		...
		#message += 'لحل المشكلة قم بعملين:    الأول: أرسل سجل الاخطاء الى المبرمج (من قائمة خدمات البرنامج) واكتب معه اسم بلدك واسم شركة الانترنيت واسماء المواقع التي لا تعمل عندك'
